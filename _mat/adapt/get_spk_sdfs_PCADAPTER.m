...
dirs.root = 'C:\KIKUCHI-LOCAL\script\2026-frontal-ramping';
dirs.raw_data = 'C:\KIKUCHI-LOCAL\data\temp\spk';

session_log = readtable(fullfile(dirs.root,'_data','session_log.csv'));
spike_log = readtable(fullfile(dirs.root,'_data','spike_log.csv'));
...
parfor session_i = 1:size(session_log,1)
    ...
    for neuron_i = 1:size(data_in.unit,2)
        ...
        [sdf.decision, raster.decision] = get_aligned_sdf(spikes, round(data_in.t_evt.resp_fix)*1000, ops); % Align units to trial event (decision)
        ...
        save(fullfile('C:\KIKUCHI-LOCAL\data\temp\proc',out_filename),'-fromstruct', data_out, '-v7.3')
