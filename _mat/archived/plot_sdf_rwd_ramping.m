%% ========================================================================
% Reward ramping: average SDF per area, split by reward probability
...
cond_labels = {'All', 'Low', 'Mid', 'High'};
cond_colors = [0.5 0.5 0.5;   % all
               0.95 0.6 0.6;  % short
               0.85 0.2 0.2;  % medium
               0.6 0.0 0.0];  % long
...
for ramp_i = 1:length(ramp_types)
    ...
    figure('Renderer', 'painters', 'Position', [200 200 700 nRows * 250]);
    sgtitle(sprintf('Reward SDF — %s neurons', label), 'FontSize', 13)

    for area_i = 1:nAreas

        % Neurons in this area with the relevant ramp flag
        area_idx = strcmp(analysis_log.area, areas{area_i}) & ...
                   analysis_log.ramping_rwd_flag == flag;
        ...
        area_data = z_reward_array(area_idx, plot_win, :);
